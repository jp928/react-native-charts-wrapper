...
//  RNCharts
//
//  Created by Jing Piao on 6/5/21.
//
import Foundation;
...
open class NectrMarker: MarkerView {
    open var color: UIColor?
    open var arrowSize = CGSize(width: 16, height: 8)
    open var font: UIFont?
    open var textColor: UIColor?
    open var minimumSize = CGSize()
    
    // bottom = to text label padding bottom + arrowHeight
    fileprivate var insets = UIEdgeInsets(top: 8.0,left: 8.0,bottom: 16.0,right: 8.0)
    fileprivate var topInsets = UIEdgeInsets(top: 14.0,left: 8.0,bottom: 16.0,right: 8.0)
    ...
    func drawRect(context: CGContext, point: CGPoint) -> CGRect{
        ...
        UIImage(named: "Dot.png")?.draw(at: CGPoint(x: point.x - 12, y: point.y - 12))
    ...
    open override func refreshContent(entry: ChartDataEntry, highlight: Highlight) {
        ...
        labelns = String(format: "%.2f kWh", (label as NSString).floatValue) as NSString
